import CssTable from '../../components/CssTable'

## Class Table

<CssTable name="checkbox" />

## Demo and Playground

[Go to Storybook](https://icestack-storybook.vercel.app/?path=/docs/data-entry-checkbox--docs)

## Css Schema

```js
{
"selector": ".checkbox",
"defaults": {
  "styled": {
    ".checkbox": {
      "apply": [
        "border-base-content",
        "rounded-btn",
        "h-6",
        "w-6",
        "cursor-pointer",
        "appearance-none",
        "border",
        "border-opacity-20"
      ],
      "css": {
        "--chkbg": "var(--base-content)",
        "--chkfg": "var(--base-400)"
      },
      "&:focus-visible": {
        "apply": [
          "outline-base-content",
          "outline",
          "outline-2",
          "outline-offset-2"
        ]
      },
      "&:checked,&[checked=\"true\"],&[aria-checked=\"true\"]": {
        "apply": [
          "bg-base-content",
          "bg-no-repeat"
        ],
        "css": {
          "animation": "checkmark var(--animation-input, 0.2s) ease-out",
          "background-image": "linear-gradient(-45deg, transparent 65%, rgba(var(--chkbg)) 65.99%),
              linear-gradient(45deg, transparent 75%, rgba(var(--chkbg)) 75.99%),
              linear-gradient(-45deg, rgba(var(--chkbg)) 40%, transparent 40.99%),
              linear-gradient(
                45deg,
                rgba(var(--chkbg)) 30%,
                rgba(var(--chkfg)) 30.99%,
                rgba(var(--chkfg)) 40%,
                transparent 40.99%
              ),
              linear-gradient(-45deg, rgba(var(--chkfg)) 50%, rgba(var(--chkbg)) 50.99%)"
        }
      },
      "&:indeterminate": {
        "apply": [
          "bg-base-content",
          "bg-no-repeat"
        ],
        "css": {
          "animation": "checkmark var(--animation-input, 0.2s) ease-out",
          "background-image": "linear-gradient(90deg, transparent 80%, rgba(var(--chkbg)) 80%),
          linear-gradient(-90deg, transparent 80%, rgba(var(--chkbg)) 80%),
          linear-gradient(
            0deg,
            rgba(var(--chkbg)) 43%,
            rgba(var(--chkfg)) 43%,
            rgba(var(--chkfg)) 57%,
            rgba(var(--chkbg)) 57%
          )"
        }
      },
      "&-primary": {
        "apply": [
          "border-primary",
          "[@media(hover:hover)]:hover:border-primary"
        ],
        "css": {
          "--chkbg": "var(--primary)",
          "--chkfg": "var(--primary-content)"
        },
        "&:focus-visible": {
          "apply": [
            "outline-primary"
          ]
        },
        "&:checked,&[checked=\"true\"],&[aria-checked=\"true\"]": {
          "apply": [
            "border-primary",
            "bg-primary",
            "text-primary-content"
          ]
        }
      },
      "&-success": {
        "apply": [
          "border-success",
          "[@media(hover:hover)]:hover:border-success"
        ],
        "css": {
          "--chkbg": "var(--success)",
          "--chkfg": "var(--success-content)"
        },
        "&:focus-visible": {
          "apply": [
            "outline-success"
          ]
        },
        "&:checked,&[checked=\"true\"],&[aria-checked=\"true\"]": {
          "apply": [
            "border-success",
            "bg-success",
            "text-success-content"
          ]
        }
      },
      "&-warning": {
        "apply": [
          "border-warning",
          "[@media(hover:hover)]:hover:border-warning"
        ],
        "css": {
          "--chkbg": "var(--warning)",
          "--chkfg": "var(--warning-content)"
        },
        "&:focus-visible": {
          "apply": [
            "outline-warning"
          ]
        },
        "&:checked,&[checked=\"true\"],&[aria-checked=\"true\"]": {
          "apply": [
            "border-warning",
            "bg-warning",
            "text-warning-content"
          ]
        }
      },
      "&-error": {
        "apply": [
          "border-error",
          "[@media(hover:hover)]:hover:border-error"
        ],
        "css": {
          "--chkbg": "var(--error)",
          "--chkfg": "var(--error-content)"
        },
        "&:focus-visible": {
          "apply": [
            "outline-error"
          ]
        },
        "&:checked,&[checked=\"true\"],&[aria-checked=\"true\"]": {
          "apply": [
            "border-error",
            "bg-error",
            "text-error-content"
          ]
        }
      },
      "&-neutral": {
        "apply": [
          "border-neutral",
          "[@media(hover:hover)]:hover:border-neutral"
        ],
        "css": {
          "--chkbg": "var(--neutral)",
          "--chkfg": "var(--neutral-content)"
        },
        "&:focus-visible": {
          "apply": [
            "outline-neutral"
          ]
        },
        "&:checked,&[checked=\"true\"],&[aria-checked=\"true\"]": {
          "apply": [
            "border-neutral",
            "bg-neutral",
            "text-neutral-content"
          ]
        }
      },
      "&:disabled": {
        "apply": [
          "bg-base-content",
          "cursor-not-allowed",
          "border-transparent",
          "opacity-20"
        ]
      }
    },
    "@keyframes checkmark": {
      "0%": {
        "css": {
          "background-position-y": "5px"
        }
      },
      "50%": {
        "css": {
          "background-position-y": "-2px"
        }
      },
      "100%": {
        "css": {
          "background-position-y": "0"
        }
      }
    }
  },
  "base": {
    ".checkbox": {
      "apply": [
        "shrink-0"
      ]
    }
  },
  "utils": {
    ".checkbox": {
      "&-xs": {
        "apply": [
          "h-4",
          "w-4"
        ]
      },
      "&-sm": {
        "apply": [
          "h-5",
          "w-5"
        ]
      },
      "&-md": {
        "apply": [
          "h-6",
          "w-6"
        ]
      },
      "&-lg": {
        "apply": [
          "h-8",
          "w-8"
        ]
      }
    }
  }
}
}
```