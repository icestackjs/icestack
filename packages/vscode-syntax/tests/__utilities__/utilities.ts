...
export async function tokenizeFile(file: string, grammarScopeName: string): Promise<Token[]> {
...
async function getGrammar(scopeName: string) {
  ...
  const registry = new tm.Registry({
    ...
    getInjections: (scope) => scope.split('.').flatMap((_, i, parts) => injections[parts.slice(0, i + 1).join('.')] || [])
