...
| ---------------------------- | ------------------ | ------ | --------------------------------------------- |
| `@b` / `@base`               | `base`             | node   | add current node selector to base             |
| `@v` / `@variant`            | `variants`         | node   | add current node selector to variants         |
| `@cv` / `@compoundVariant`   | `compoundVariants` | node   | add current node selector to compoundVariants |
| `@dv` / `@defaultVariant`    | `defaultVariants`  | global | define defaultVariants                        |
| `@gb` / `@gbase`             | `base`             | global | define base                                   |
| `@gv` / `@gvariant`          | `variants`         | global | define variants                               |
| `@gcv` / `@gcompoundVariant` | `compoundVariants` | global | define defaultVariants                        |
